...
void Create_new_Generation_CascodedOTA_with_different_ICs(int IndextFirstParent,int IndexSecondParent,	struct CascodedOTA CascodedOTA_Designs[],int NumberOfChilds,int lowerIbias,int upperIbias,int lowerLength, int upperLength,double IC_Ranges_CurrentMirrorOTA[]);
	...
	struct data_sheet{
		...
		double LowerIbias_same_PowerConsuption;
...
//Global Variables
	...
	double LowerIbias_same_PowerConsuption = data_sheet.LowerIbias_same_PowerConsuption;
...
double Ibias_Table[26];	
double PowerConsuptionMeasurments;
PowerConsuptionMeasurments = LowerIbias_same_PowerConsuption*VDD;
double VDD_measurments = VDD;
...
index_Ibias = -1;
while(VDD_measurments >= 1.25) {
	
	index_Ibias = index_Ibias+1;
	
//	Ibias_Table[index_Ibias] = index_Ibias*2 + 2;
	Ibias_Table[index_Ibias] = PowerConsuptionMeasurments/VDD_measurments;
//	*VDD_Control = *VDD_Control - 0.05; 
	VDD_measurments = VDD_measurments - 0.05;
//	VDD = VDD_measurments;
...
if(Mode == 0 || Mode == 2){	
	...
	printf("\n\ndata_sheet.VSS:%g\n",data_sheet.VSS); 
	printf("\n\ndata_sheet.CL:%g\n",data_sheet.CL); 
	printf("\n\ndata_sheet.LowerIbias_same_PowerConsuption:%g\n",data_sheet.LowerIbias_same_PowerConsuption); 
			...
			for (j=0;j<15;j=j+2){
				...
				if(Propability<50){
					...
					if(j==0){
						...
						CascodedOTA_Designs[i].Mosfets[16].L = CascodedOTA_Designs[IndextFirstParent].Mosfets[16].L;
						CascodedOTA_Designs[i].Mosfets[16].lamdaC = CascodedOTA_Designs[IndextFirstParent].Mosfets[16].lamdaC;
						CascodedOTA_Designs[i].Mosfets[16].IC_WithOutVS = CascodedOTA_Designs[IndextFirstParent].Mosfets[16].IC_WithOutVS;
						CascodedOTA_Designs[i].Mosfets[16].IC = CascodedOTA_Designs[IndextFirstParent].Mosfets[16].IC;
					}
					else if(j==2){
